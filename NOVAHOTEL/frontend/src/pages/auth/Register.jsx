...
/**
 * Trang đăng ký Nova Hotel
 * - Layout sang trọng, dễ điền form
...
export default function Register() {
  ...
  const validate = () => {
    ...
    if (password !== confirm) nextErrors.confirm = 'Mật khẩu xác nhận chưa khớp'
  ...
  return (
    <main className="min-h-screen px-4 py-8 md:px-6 md:py-10">
      <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-7xl overflow-hidden rounded-[36px] border border-white/70 bg-white/85 shadow-[0_30px_100px_rgba(15,23,42,0.12)] backdrop-blur-xl lg:grid-cols-[1fr_1fr]">
          ...
          <div className="relative flex h-full flex-col justify-between">
            <div>
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm text-blue-50 backdrop-blur">
                <Sparkles size={15} className="text-amber-300" /> Tạo tài khoản Nova Hotel
              </div>
              <h1 className="max-w-xl text-4xl font-bold tracking-tight md:text-6xl">
                Một tài khoản để quản lý toàn bộ hành trình đặt phòng.
              </h1>
              <p className="mt-5 max-w-xl text-base leading-8 text-blue-100 md:text-lg">
                Đăng ký nhanh chóng để lưu lịch sử đặt phòng, đánh giá phòng và theo dõi các trải nghiệm lưu trú của bạn.
              </p>
            </div>

            <div className="mt-10 grid gap-3 sm:grid-cols-3">
              {[
                ['Nhanh', 'Tạo tài khoản trong vài giây'],
                ['Gọn', 'Biểu mẫu tối giản, dễ hiểu'],
                ['An toàn', 'Dữ liệu được bảo vệ rõ ràng'],
              ].map(([title, desc]) => (
          ...
          <div className="w-full">
            <div className="mx-auto max-w-md rounded-[30px] border border-slate-200 bg-white p-7 shadow-[0_24px_80px_rgba(15,23,42,0.08)] md:p-8">
              <div className="mb-8 text-center">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-500">Tạo tài khoản để bắt đầu trải nghiệm dịch vụ.</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-5">
                <Field label="Họ và tên" error={errors.name} icon={User}>
                  <input
                    value={name}
                    onChange={(e) => {
                ...
                <Field label="Xác nhận mật khẩu" error={errors.confirm} icon={Lock}>
                  <input
                    type={showPassword ? 'text' : 'password'}
                    value={confirm}
                    onChange={(e) => {
                ...
                <div className="flex items-center justify-between text-sm">
                  <Link to="/login" className="font-semibold text-blue-700 transition hover:text-blue-800">Đã có tài khoản? Đăng nhập</Link>
                </div>

                <Button type="submit" className="w-full py-3" disabled={loading}>
                  {loading ? (
                    <span className="flex items-center gap-2"><LoadingSpinner size={0.8} /> Đang đăng ký...</span>
                  ) : (
                    'Đăng ký'
                  )}
                </Button>
